...
export class PropositionController {
  ...
  public createProposition(req: Request, res: Response) {
    ...
    const proposition = new Proposition({ subject:jobTitle , proposedPrice:hourlyRate ,coverLetter:jobDescription, userId, demandId });
  ...
  public AcceptProposition(req: Request, res: Response) {
    const propositionId = req.params.id;
  ...
  public RefuseProposition(req: Request, res: Response) {
    const propositionId = req.params.id;

    Proposition.findByIdAndUpdate(propositionId, { refusalStatus:1 }, { new: true })
      .then((proposition: IProposition | null) => {
        if (proposition) {
          res.status(200).json(proposition);
        } else {
